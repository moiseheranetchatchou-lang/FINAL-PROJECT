...
(Place screenshots in README before submission)
...
Document name: Tp_final_Typescript
...
13) Suggestions to ameliorate 

- Sauvegarde locale via `localStorage` (si autorisé par l'enseignant).
- Responsive & plus joli UI (tailwind / custom CSS — attention: frameworks UI non autorisés ? Vérifier consignes).
- Export des inscriptions en CSV.
- Ajout d'un mini calendrier pour choisir la date plus facilement.